...
class CarState(CarStateBase):
  def __init__(self, CP):
    ...
    self.prev_has_vehicle_ahead = False
  ...
  def update(self, cp, cp_cam):
    ...
    ret.accCmd = cp.vl["CRZ_INFO"]["ACCEL_CMD"] / 4096 * 100 if cp.vl["CRZ_CTRL"]["CRZ_ACTIVE"] else 0.0
    ...
    # MRCC vehicle ahead
    ret.hasVehicleAhead = cp.vl["CRZ_CTRL"]["RADAR_HAS_VEHICLE_AHEAD"] == 1

    # Check if it is the edge of first radar contact    ret
    self.radar_contact = True if ret.hasVehicleAhead and not self.prev_has_vehicle_ahead else False
    ret.radarContact = self.radar_contact
    self.prev_has_vehicle_ahead = ret.hasVehicleAhead
  ...
  def get_can_parser(CP):
    ...
    if CP.carFingerprint in GEN1:
      signals += [
        ...
        ("LEFT_BS1", "BSM"),
        ("RIGHT_BS1", "BSM"),
        ("RADAR_HAS_VEHICLE_AHEAD", "CRZ_CTRL"),
      ]
